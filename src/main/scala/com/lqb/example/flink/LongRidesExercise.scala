...
import sources.TaxiRideGenerator
...
object LongRidesExercise {
  ...
  class LongRidesJob(source: SourceFunction[TaxiRide], sink: SinkFunction[Long]) {
    ...
    def execute(): JobExecutionResult = {
      ...
      val watermarkStrategy: WatermarkStrategy[TaxiRide] = WatermarkStrategy
  ...
  def main(args: Array[String]): Unit = {
    val job: LongRidesJob = new LongRidesJob(new TaxiRideGenerator, new PrintSinkFunction)
  ...
  class AlertFunction extends KeyedProcessFunction[Long, TaxiRide, Long] {
    ...
    override def open(parameters: Configuration): Unit = {
      state = getRuntimeContext.getState(new ValueStateDescriptor[TaxiRide]("key state", classOf[TaxiRide]))
    }

    def getDealTimer(ride: TaxiRide): Long = {
      ride.eventTime.toEpochMilli + 2.hours.toMillis
    }

    def overLongRideTime(ride: TaxiRide, first: TaxiRide): Boolean = {
      Duration.between(ride.eventTime, first.eventTime).compareTo(Duration.ofHours(2)) > 0
    }
    ...
    override def processElement(
      ...
      val first: TaxiRide = state.value()
      if (first == null) {
        state.update(ride)
        if (ride.isStart) {
          context.timerService().registerEventTimeTimer(getDealTimer(ride))
        }
      } else {
        if (ride.isStart) {
          if (overLongRideTime(ride, first)) {
            out.collect(ride.rideId)
          }
        } else {
          context.timerService().deleteEventTimeTimer(getDealTimer(first))
          if (overLongRideTime(ride, first)) {
            out.collect(ride.rideId)
          }
        }
    ...
    override def onTimer(
      ...
      out.collect(state.value().rideId)
      state.clear()
    }
